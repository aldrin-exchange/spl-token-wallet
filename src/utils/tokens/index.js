...
export async function getOwnedTokenAccounts(connection, publicKey) {
  ...
  return resp
    .map(({
      pubkey, account: {
        data, executable, owner, lamports,
      },
    }) => ({
...
export async function createAndInitializeTokenAccount({
  ...
  return signAndSendTransaction(connection, transaction, payer, signers)
}
...
export async function transferTokens({
  ...
  const destinationSplTokenAccount = (
    ...
    .map(({ publicKey, accountInfo }) => ({ publicKey, parsed: parseTokenAccountData(accountInfo.data) }))
    .filter(({ parsed }) => parsed.mint.equals(mint))
    .sort((a, b) => b.parsed.amount - a.parsed.amount)[0]
