...
export class Wallet {
  ...
  getTokenAccountInfo = async () => {
    const accounts = await getTokenAccountsByOwner(this.connection, this.publicKey)
    return accounts
      .map(({ publicKey, accountInfo }) => {
        setInitialAccountInfo(this.connection, publicKey, accountInfo)
        return { publicKey, parsed: parseTokenAccountData(accountInfo.data) }
      })
      .sort((account1, account2) => account1.parsed.mint
        .toBase58()
        .localeCompare(account2.parsed.mint.toBase58()))
  }

  createTokenAccount = async (tokenAddress) => createAndInitializeTokenAccount({
  ...
  createAssociatedTokenAccount = async (splTokenMintAddress) => createAssociatedTokenAccount({
    connection: this.connection,
    wallet: this,
    splTokenMintAddress,
  })

  tokenAccountCost = async () => this.connection.getMinimumBalanceForRentExemption(
    ACCOUNT_LAYOUT.span,
  )
  ...
  transferSol = async (destination, amount) => nativeTransfer(this.connection, this, destination, amount)

  closeTokenAccount = async (publicKey, skipPreflight = false) => closeTokenAccount({
    connection: this.connection,
    owner: this,
  ...
  signTransaction = async (transaction) => this.provider.signTransaction(transaction)

  createSignature = async (message) => this.provider.createSignature(message)
}
...
export var WalletProvider = ({ children }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!wallet) {
        const account = walletSelector.walletIndex !== undefined
          ? getAccountFromSeed(
  ...
  function addAccount({ name, importedAccount, ledger }) {
    if (importedAccount === undefined) {
      ...
      const newPrivateKeyImports = { ...privateKeyImports }
  ...
  const getWalletNames = () => JSON.stringify(
    [...Array(walletCount).keys()].map((idx) => localStorage.getItem(`name${idx}`)),
  )
  ...
  const [accounts, derivedAccounts] = useMemo(() => {
    ...
    const derivedAccounts = [...Array(walletCount).keys()].map((idx) => {
      const address = getAccountFromSeed(seedBuffer, idx, derivationPath)
        .publicKey
      const name = localStorage.getItem(`name${idx}`)
      return {
        selector: {
          walletIndex: idx,
          importedPubkey: undefined,
...
export function useWalletPublicKeys() {
  ...
  publicKeys = useRefEqual(
    publicKeys,
    (oldKeys, newKeys) => oldKeys.length === newKeys.length
...
export function useBalanceInfo(publicKey) {
  ...
  const { name, symbol, logoUri } = useTokenInfo(mint)
