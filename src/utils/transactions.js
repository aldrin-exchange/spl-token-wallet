...
import {
  Message, SystemInstruction, SystemProgram, PublicKey,
} from '@solana/web3.js'
import {
  decodeInstruction,
...
export const decodeMessage = async (connection, wallet, message) => {
  ...
  for (let i = 0; i < transactionMessage.instructions.length; i += 1) {
...
const toInstruction = async (
  ...
  try {
    if (programId.equals(SystemProgram.programId)) {
      ...
      const decodedInstruction = decodeTokenInstruction(decoded)
    ...
    } if (programId.equals(RAYDIUM_STAKE_PROGRAM_ID)) {
    ...
    } if (programId.equals(RAYDIUM_LP_PROGRAM_ID)) {
    ...
    } if (programId.equals(MANGO_PROGRAM_ID) || programId.equals(MANGO_PROGRAM_ID_V2)) {
...
// TODO
const decodeMangoInstruction = () => undefined
const decodeStakeInstruction = () => undefined
const decodeLpInstruction = () => undefined
const handleDexInstruction = async (
  ...
  // get market info
  const marketInfo = accountKeys
    && MARKETS.find(
      (market) => accountKeys.findIndex((accountKey) => accountKey.equals(market.address)) > -1,
    )
  ...
  let market; let
    programIdAddress
  try {
    const marketAddress = marketInfo?.address || getAccountByIndex(accounts, accountKeys, 0)
    programIdAddress = marketInfo?.programId
